...
int SimdCalculator::calculateChunk(
    ...
    for (int i = 0; i < count; ++i) {
        if (effWeights[i] <= 0) {
            if (globalRules.noWeightDefaultPrice > 0 && freights[i] > 0) {
                double result = freights[i];
                result = CalculationRule::applyPriceIncreases(result, 0, globalRules.activityRules, orders[i].businessTime);
                result = CalculationRule::applyPriceIncreases(result, 0, globalRules.tempPriceIncreases, orders[i].businessTime);
                QString stdProvince = ProvinceUtils::standardize(orders[i].destinationProvince);
                for (const PriceIncreaseRule &ppi : globalRules.provincePriceIncreases) {
                    if (!ppi.isActive) continue;
                    if (CalculationRule::provinceMatches(stdProvince, ppi.province)) {
                        if (ppi.mode != IncreaseMode::PerKg)
                            result = getIncreaseFunc(ppi.mode)(result, 0, ppi.amount);
                    }
                }
                freights[i] = std::round(result * 100.0) / 100.0;
                orders[i].usedRule = QStringLiteral("无重量默认价");
            }
            continue;
        }
